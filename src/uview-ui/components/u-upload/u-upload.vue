<template>
	<view class="u-upload" v-if="!disabled">
		...
		>
			...
			></u-line-progress>
			<view @tap.stop="retry(index)" v-if="item.error" class="u-error-btn">{{$t('点击重试')}}</view>
		...
		<view style="display: inline-block;" @tap="selectFile" v-if="maxCount > lists.length">
			...
			>
				...
				<view class="u-add-tips">{{ $t(uploadText) }}</view>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		// 删除一个图片
		deleteItem(index) {
			var elements = document.querySelectorAll('.uni-modal');
			  elements.forEach(function(element) {
			    element.style.backgroundColor = '#1e1739';
			  });
			uni.showModal({
				cancelColor: '#fff',
				content:  this.$t('您确定要删除此项吗'),
