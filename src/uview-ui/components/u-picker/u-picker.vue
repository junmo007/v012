<template>
	<u-popup :maskCloseAble="maskCloseAble" mode="bottom" :popup="false" v-model="value" length="auto"
		:safeAreaInsetBottom="safeAreaInsetBottom" @close="close" :z-index="uZIndex" borderRadius="32">
		<view class="u-datetime-picker">
			<view class="u-picker-header" @touchmove.stop.prevent="">
				<view class="u-btn-picker u-btn-picker--tips" :style="{ color: cancelColor }" hover-class="u-opacity"
					:hover-stay-time="150" @tap="getResult('cancel')">{{$t(cancelText)}}</view>
				<view class="u-picker__title">{{ $t(title) }}</view>
			...
			<view class="u-picker-body">
				...
				<picker-view v-else-if="mode == 'time'" :value="valueArr" @change="change" class="u-picker-view"
					...
					<!-- 日份 -->
					<picker-view-column v-if="!reset && params.day && isShowDay">
			...
			<view class="w_100 u-text-center u-p-t-30 u-p-b-30">
				<view class="pickerBtn" @tap.stop="getResult('confirm')">{{$t('public6')}}</view>
			</view>
		</view>
	</u-popup>
</template>
...
<style lang="scss" scoped>
	...
	.u-picker-header {
		...
		box-sizing: border-box;
		font-size: 30rpx;
		background: var(--u-picker-head-bg);
		position: relative;
	}
	...
	.u-picker-body {
		...
		height: 500rpx;
		overflow: hidden;
		background: var(--u-picker-body-bg);
