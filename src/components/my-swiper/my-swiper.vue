		...
		<view class="card-loop">
			<transition-group>
				<view v-for="(item, idx) in loopModules" :key="idx" class="loop-item" :style="item.style"
					...
					<image :src="baseData[idx].img" class="zSwiperListImage" mode="aspectFill" @click.stop="jump(item)">
					</image>
				</view>
			</transition-group>
		</view>
		<view class="u-flex u-row-center card-loop-center">
			<view v-for="(item,index) in arr" :key="`loopCenterIdx${index}`" class="centerClass"
				:class="index == loopCenterIdx ? 'active' : ''"></view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			jump(item) {
				if(item.desc.url){
					// window.location.href = item.desc.url;
					this.$go(item.desc.url);
				}
			},
			// 初始化轮播
			initLoop() {
