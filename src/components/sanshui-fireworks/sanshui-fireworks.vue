...
*/
	const minBrowserRefreshTime = window && (window.requestAnimationFrame || window.webkitRequestAnimationFrame || window
		...
		methods: {
			...
			*/
			...
			initCanvas() {
				...
				fireCanvasBox.save();
				setTimeout(() => { fireCanvasBox.draw(); }, 50)
				this.fireworksDraw();
			},
			fireworksDraw() {
				let ribbon = [];
				...
				for (; particleCount--;) {
					...
					ribbon.push({ // 菜单位置初始化
						...
						color: (i = (n.color + '').replace(/[^0-9a-f]/gi, ''), i.length < 6 && (i = i[0] + i[0] +
				...
				minBrowserRefreshTime(function drawRibbon() {
					...
					fireCanvasBox.draw(),
						...
						ribbon = ribbon.filter((e) => {
							...
							// 开始画图
							fireCanvasBox.fillStyle = 'rgba(' + e.color.r + ', ' + e.color.g + ', ' + e.color
								.b + ', ' + (1 - (e.tick++) / e.totalTicks) + ')',
								fireCanvasBox.beginPath(),
								fireCanvasBox.moveTo(Math.floor(e.x), Math.floor(e.y)),
								fireCanvasBox.lineTo(Math.floor(e.wobbleX), Math.floor(e.y + e.random * e
									.tiltSin)),
								fireCanvasBox.lineTo(Math.floor(e.wobbleX + e.random * e.tiltCos), Math.floor(e
									.wobbleY + e.random * e.tiltSin)),
								fireCanvasBox.lineTo(Math.floor(e.x + e.random * e.tiltCos), Math.floor(e
									.wobbleY)),
