...
<template>
	<view class="date">
		...
		<swiper :style="{height:(retract ? 2 * 30 : (week_list.length + 1) * 30 ) + 'rpx'}" :current="current" circular @change="change_date">
			<swiper-item>
	...
	methods:{
		change(){
			let value = {
				...
				olddate: this.old_date.replace(/-(\d)(?!\d)/g, '-0$1')
			};
...
<style lang="scss">
// @import '../../static/lx-calendar/fonts/iconfont.css';
