<template>
	<view class="content" :data-theme="themeMode">
		<view class="container" :class="sideshow?'is-mask':''">
			<uniNavHeader isback :title="$t('agent27')" @clickLeft="menuClick"></uniNavHeader>
			<!-- sidebar -->
					...
					<template v-if="showEmpty == 1">
						<view class="">
							<uni-table class="fbd-table" emptyText="">
								<!-- 表头行 -->
								<uni-tr>
									<uni-th align="left">{{$t('agent62')}}/{{$t('user32')}}</uni-th>
									<uni-th align="right">{{$t('user33')}}</uni-th>
								</uni-tr>
								<!-- 表格数据行 -->
								<uni-tr v-for="(item,index) in dataList" :key="`list${index}`">
									<uni-td>{{item.username}}/{{item.zd_number}}</uni-td>
									<uni-td align="right">{{item.tz_price}}</uni-td>
								</uni-tr>
							</uni-table>
						</view>
