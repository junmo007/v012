<template>
	<view class="content" :data-theme="themeMode">
		<view class="container" :class="sideshow?'is-mask':''">
			<uniNavHeader isback :title="$t('agent29')" @clickLeft="menuClick"></uniNavHeader>
			<!-- sidebar -->
			...
			<view class="bs-container">
				<view class="bs-content">
					<template v-if="showEmpty == 1">
						<view class="panel">
							<view class="u-flex u-row-between u-p-t-24 u-p-b-24 u-p-l-20 u-p-r-20 activeCar">
								<view>{{$t('agent62')}}/{{$t('user32')}}</view>
								<view>{{$t('user33')}}</view>
							</view>
							<view v-for="(item,index) in dataList" :key="`list${index}`"
								:class="index%2!=0?'activeCar':''">
								<view class="u-flex u-flex-nowrap u-row-between u-p-t-24 u-p-b-24 u-p-l-20 u-p-r-20">
									<view>{{item.username}}/{{item.order_sn}}</view>
									<view>{{item.apply_price}}</view>
								</view>
							</view>
						</view>
